...
Neste trabalho, são investigados escoamentos incompressíveis, isotérmicos e com contornos móveis. As seções seguintes apresentam a abordagem adotada para a resolução desse tipo de problema, bem como sua implementação computacional. Adota-se uma descrição ALE para representar as equações, enquanto a discretização espacial é realizada por meio do MEF ou da AIG.

Para tratar questões numéricas recorrentes nesse tipo de equações, como as oscilações espúrias em casos de convecção dominante, típicas da aplicação direta do método dos resíduos ponderados na formulação clássica de Galerkin, emprega-se a metodologia SUPG. Além disso, a estabilização PSPG é aplicada com o objetivo de contornar as condições de LBB associadas aos escoamentos incompressíveis, permitindo o uso estável de um mesmo espaço de funções de aproximação para pressão e velocidade. A integração temporal é realizada por meio do método $\alpha$-generalizado.
...
\begin{align}
	\begin{split}
	\frac{\partial \density}{\partial t}\deriv V =& \left(\density u_1 \deriv A_{1}  +  \density u_2 \deriv A_{2} + \density u_3  \deriv A_{3} \right) - \left(\left(\density u_1 + \frac{\partial \density u_1}{\partial y_1}\deriv y_1 \right)\deriv A_{1} \right. + \\ & \left. \left(\density u_2+ \frac{\partial \density u_2}{\partial y_2}\deriv y_2 \right) \deriv A_{2} + \left(\density u_3 + \frac{\partial \density u_3}{\partial y_3}\deriv y_3 \right) \deriv A_{3}\right), \label{eq:conser_massa_0} 
	\end{split}
\end{align}
\noindent com $\density$ sendo massa específica do fluido e $\deriv A_{i}$ a área referente à face ortogonal ao eixo $y_i$. Considerando que $\deriv V = \deriv y_1 \deriv y_2 \deriv y_3 = \deriv y_1 \deriv A_1 = \deriv y_2 \deriv A_2 = \deriv y_3 \deriv A_3 $  e manipulando-se algebricamente a Equação~\eqref{eq:conser_massa_0}, resulta:
\begin{align}
...
\begin{align}
	\begin{split}
	\frac{\partial \density \velocity}{\partial t}\deriv V =& u_1\density \velocity \deriv A_1 + u_2\density \velocity \deriv A_2 + u_3 \density \velocity \deriv A_3 - \left(\left(u_1 \density \velocity + \frac{\partial u_1 \density \velocity}{\partial y_1}\deriv y_1 \right)\deriv A_1 \right. + \\ & \left.  \left(u_2 \density \velocity + \frac{\partial u_2 \density \velocity}{\partial y_2}\deriv y_2 \right)\deriv A_2 + \left(u_3 \density \velocity + \frac{\partial u_3 \density \velocity}{\partial y_3}\deriv y_3 \right)\deriv A_3\right) + \mathbf{q} \deriv V.
	\label{eq:QM_0} 
	\end{split}
\end{align}	
...
Dividindo-se a Equação~\eqref{eq:QM_0} por $\deriv V$ e efetuando-se as subtrações, chega-se a:
\begin{align}
...
\section{Descrição euleriana-lagrangiana arbitrária} \label{capitulo:Cap2:ALE}

A descrição lagrangiana-euleriana arbitrária \cite{DoneaGH:1982,HughesLZ:1981} representa uma generalização das descrições puramente lagrangiana e puramente euleriana do movimento do contínuo. A descrição lagrangiana fixa a atenção em pontos materiais do contínuo, enquanto na descrição euleriana considera-se uma porção fixa do espaço ocupada pelo contínuo e analisam-se os pontos materiais que passam por essa porção ao longo do tempo. Como consequência, na descrição puramente lagrangiana a malha computacional move-se com o contínuo, enquanto, na euleriana, a malha computacional mantém-se espacialmente fixa e permeável ao meio contínuo. Por sua vez, na descrição lagrangiana-euleriana arbitrária trabalha-se com um domínio de referência que pode mover-se de maneira independente do movimento dos pontos materiais do contínuo analisado.
...
\begin{figure}[!htbp]
	\caption{Domínios na descrição lagrangiana-euleriana arbitrária}
	\begin{center}
...
Considera-se, neste texto, o domínio de referência $\domainRef$, como sendo a configuração inicial da malha, enquanto as configurações atuais, tanto da malha como do contínuo, coincidem com a referência espacial $\domain$.
...
Da mesma forma, o domínio de referência é mapeado a partir do inicial por:
\begin{align}
	\posALE = \fmapRI(\posMat,t).
\end{align}

A velocidade dos pontos da malha é calculada através de:
\begin{align}
	\velocityALE = \left. \frac{\partial \fmapAR(\posALE,t)}{\partial t} \right|_{\posALE},
\end{align}
\noindent e a velocidade dos pontos materiais no instante $t$ é obtida pela derivada do vetor posição $\pos$, mantendo $\posMat$ fixo, como:
\begin{align}
...
Considerando que $\fmapAI\left(\posMat,t\right) = \fmapAR \circ \fmapRI$, define-se:
\begin{align}
...
Dessa forma, é estabelecida uma relação entre a velocidade da malha e a velocidade do ponto material:
\begin{align}
	\velocity = \velocityALE + \frac{\partial{\pos}}{\partial{\posALE}}\cdot \mathbf{w}. \label{eq:vel_rel_mat_ALE}
\end{align}

Supondo agora uma grandeza física escalar, denominada $g(\pos,t)$ na configuração espacial, de $g^{*}(\posALE,t)$ na configuração de referência e $g^{**}(\posMat,t)$ na configuração material, é possível escrever:
\begin{align}
...
A partir dessa expressão escreve-se a derivada temporal da variável na configuração material:
\begin{align}
	\frac{\partial g^{**}}{\partial t} = \frac{\partial g}{\partial t} + \frac{\partial g}{\partial \pos} \cdot \velocity, 
\end{align}
\noindent que corresponde à derivada material de g. Para facilitar a visualização, removem-se os sobrescritos $**$, obtendo-se:
\begin{align}
...
Usando essa mesma metodologia, a transformação de $g^{*}(\posALE,t)$ para a referência material é escrita da seguinte forma:
\begin{align}
	g^{**} = g^{*}  \circ \fmapRI,
\end{align}
\noindent que resulta no gradiente:
\begin{align}
	\begin{bmatrix}
		\frac{\partial {g^{**}}}{\partial {\posMat}} & \frac{\partial {g^{**}}}{\partial t} \\
	\end{bmatrix}
	...
	\end{bmatrix},
\end{align}
\noindent com a segunda coluna expressa por:
\begin{align}
	\frac{\partial g^{**}}{\partial t} = \frac{\partial g^*}{\partial t} + \frac{\partial g^*}{\partial \posALE} \cdot \mathbf{w}. \label{eq:ref_mat}
\end{align}

Utilizando-se a Equação~\eqref{eq:vel_rel_mat_ALE} e substituindo-a na Equação~\eqref{eq:ref_mat}, tem-se:
\begin{align}
...
Para expressar a equação da quantidade de movimento em uma descrição ALE, basta substituir na Equação~\eqref{eq:Navier-Stokes_der_mat_Euleriana} a definição de derivada material apresentada na Equação~\eqref{eq:der_mat_ALE}, resultando:
\begin{align}
...
\begin{figure}[!htbp]
	\caption{Domínio para um problema da DFC}
	\begin{center}
		\includegraphics[scale=3.0,trim=0cm 0.1cm 0cm 0.05cm, clip=true]{Imagens/Cap2/dominioFluido.pdf}	
	\end{center}
	\legend{Fonte: Elaborada pela autora}
...
\end{align}
\noindent sendo $\left(H^{1}\left(\domain\right)\right)^{\nsd}$ o Espaço de Hilbert de ordem $1$ e $L^{2}\left(\domain\right)$ o Espaço de Lebesgue de funções quadrado integráveis.
...
A solução do problema consiste, então, em encontrar $\velocity \in \usolution$ e $\press \in \psolution$, de modo que para todo $\utest \in \uweighting$ e para todo $\ptest \in \pweighting$, a Equação~\eqref{eq:QM_forma_fraca_0_cap2} e a Equação~\eqref{eq:C_forma_fraca_0_cap2} sejam verdadeiras.

\subsection{Método dos Elementos Finitos} \label{capitulo:Cap2:FormaFraca:ElementosFinitos}

Antes de prosseguir com a discretização espacial da forma fraca do conjunto de equações da Mecânica dos Fluidos, é fundamental compreender os princípios básicos do Método dos Elementos Finitos.  A discretização espacial tanto pelo MEF, como pela AIG (\autoref{capitulo:Cap3}), baseia-se em tomar um problema contínuo com domínio $\domain$ e representá-lo por um domínio $\domainh$ que é dividido em subdomínios $\domainE$, também chamados de elementos (MEF) ou células (AIG), de forma que:
\begin{align}
	\domain \approx \domainh = {\bigcup_{e = 1}^{\nel}} \domainE,
\end{align}
\noindent onde $\nel$ representa o número total de elementos ou células.

Da mesma forma, o contorno de $\domain$ é aproximado da seguinte forma:
\begin{align}
	\boundary \approx \boundaryh = {\bigcup_{b = 1}^{\neb}} \boundary^{b},
\end{align}
\noindent onde $\neb$ representa o número de faces ou lados, de elementos ou células, que formam o contorno.

No MEF, cada elemento é definido com auxílio de um conjunto de pontos chamados nós. As variáveis de interesse do problema, que incluem a geometria na abordagem isoparamétrica, são aproximadas pela combinação linear de um número finito de funções associadas aos nós, chamadas funções de forma, multiplicadas por variáveis denominadas parâmetros nodais. As funções de forma utilizadas no MEF tradicionalmente satisfazem a propriedade de partição da unidade, ou seja, a soma das funções de forma associadas a todos os nós de um elemento resulta em 1 para qualquer ponto dentro do seu domínio. A técnica de elementos finitos pode ser encontrada nos diversos livros disponíveis sobre o assunto, tais como \citeonline{Reddy:2006} e \citeonline{ZienkiewiczTZ:2013}.

Neste trabalho, para a Mecânica dos Fluidos no contexto do MEF, são utilizadas funções de forma quadráticas do tipo polinômios de Lagrange, sendo empregados elementos isoparamétricos triangulares para o caso 2D e tetraédricos para o caso 3D. Esses elementos são apresentados na Figura~\ref{fig:elementoFinito2d} e Figura~\ref{fig:elementoFinito3d}, onde são ilustrados também as coordenadas paramétricas adimensionais adotadas para definir as funções de forma. 
...
\noindent sendo que para uma geometria tridimensional o vetor $\pos$ possui coordenadas $y_1,y_2$ e $y_3$, as quais representam as posições atuais dos pontos que compõem o domínio, e o vetor das coordenadas paramétricas $\coordAdimen$ é composto por $\xsi$, $\eta$ e $\zeta$.  O subíndice "$a$" $ \ $ representa o índice dos nós da malha, $\nnos$ o número total de nós e $\shapef$ as funções de forma da discretização.


\subsection{Discretização espacial} \label{capitulo:Cap2:DiscEspacial}

As funções-tentativa para velocidade e pressão, bem como as funções-teste, são definidas pela combinação linear das funções de forma sobre cada subdomínio por:
\begin{align}
	\velocityh(\adimensionalcoordinates,t) = \sum_{a = 1}^{\nnos} \velocity_{a}(t)\shapef_{a}(\adimensionalcoordinates), \label{eq:interp_vel}
\end{align}
\begin{align}
	\pressh(\adimensionalcoordinates,t)  = \sum_{a = 1}^{\nnos} \press_{a}(t)\shapef_{a}(\adimensionalcoordinates),\label{eq:interp_press} 
\end{align}
\begin{align}
	\utesth(\adimensionalcoordinates)  = \sum_{a = 1}^{\nnos} \utest_{a}\shapef_{a}(\adimensionalcoordinates) \label{eq:interp_utest}
\end{align}
\noindent e
\begin{align}
	\ptesth(\adimensionalcoordinates)  = \sum_{a = 1}^{\nnos} \ptest_{a}\shapef_{a}(\adimensionalcoordinates). \label{eq:inter_ptest} 
\end{align}
...
No entanto, as formulações obtidas pelo método de Bubnov-Galerkin são conhecidas por apresentarem oscilações espúrias em escoamentos dominados pela convecção. Uma das formas de se lidar com esse problema é a utilização de métodos estabilizados, como o SUPG \cite{BrooksH:1982, HughesT:1984}. Essa metodologia consiste em adicionar à equação da quantidade de movimento, o seu resíduo ponderado por $\SUPG \left(\left(\velocityh - \velocityALEh \right) \cdot \gradient \utesth\right)$, onde $\SUPG$ é um parâmetro calculado para garantir bom condicionamento ao sistema. Do ponto de vista numérico, isso adiciona termos estabilizantes na direção das linhas de corrente.

Para os problemas de escoamentos incompressíveis aqui analisados, deve-se levar em conta que os campos de velocidade e pressão não podem ser aproximados arbitrariamente, podendo levar à ocorrência de oscilações espúrias no campo de pressão e instabilidade na solução. Para evitar isso, podem ser escolhidos elementos Taylor-Hood que obedeçam à condição de LBB \cite{BrezziF:1991,StrangF:2008,ZienkiewiczTN:2014}, ou pode-se recorrer a um método estabilizado. 

Para conferir maior flexibilidade ao programa, opta-se pela formulação estabilizada com a técnica PSPG \cite{HughesFB:1986,TezduyarMRS:1992a}. Essa técnica consiste em adicionar à equação da continuidade, o resíduo da equação da quantidade de movimento ponderado pela função $\PSPG \left(\frac{\gradient \ptesth}{\density}\right)$, onde $\PSPG$ é um parâmetro calculado para garantir bom condicionamento ao sistema. Essa metodologia introduz um termo dependente da pressão na equação da continuidade, que é responsável pela estabilização, contornando a condição LBB.

Por fim, para prover maior estabilização em problemas com formação de vórtices, adiciona-se à equação da quantidade de movimento o resíduo da equação da continuidade ponderado por $ \LSIC \density \left(\divergence \cdot \utesth\right)$, sendo $\LSIC$ um parâmetro de estabilização. Essa estabilização, denominada LSIC, dá origem a um termo do tipo mínimos quadrados que introduz, de forma consistente, maior estabilidade à formulação \cite{BazilevsTT:2013a,TezduyarO:2000}.
...
Assim, o problema da dinâmica dos fluidos na formulação estabilizada, passa a ser: determinar $\velocityh \in \usolutionh$ e $\pressh \in \psolutionh$, de tal modo que $\forall$ $\utesth \in \uweightingh$ e $\ptesth \in \pweightingh$ as seguintes expressões sejam verdadeiras:
\begin{align}
	\begin{split}
...
Visto que existem funções teste separadas para a velocidade e pressão, pode-se definir dois vetores residuais correspondentes à equação da quantidade de movimento $\NNSM$ e à equação da continuidade $\NNSC$. Considerando a arbitrariedade de $\utest_{a}$ e $\ptest_{a}$, tem-se:
\begin{align}
...
\begin{align}
	\begin{split}
		\left(\NNSM\right)_{a,i} =&\int_{\domain} \shapef_{a}\mathbf{e}_i \cdot \density\left(\left. \frac{\partial\velocityh}{\partial t}\right|_{\posALE}+\left(\velocityh - \velocityALEh \right)\cdot \gradient \velocityh - \sbodyforceh\right) \deriv \domain +\int_{\domain} \straintensor \left(\shapef_{a}\mathbf{e}_i\right) : \stressTensor \left(\velocityh,\pressh\right)\ \deriv \domain\\ &
		- \int_{\boundaryN}\shapef_{a}\mathbf{e}_i \cdot \surfaceLoadh \ \deriv \boundaryN 
		+ \sum_{e=1}^{\nel} \int_{\domainE} \SUPG \left(\left(\velocityh - \velocityALEh \right) \cdot \gradient \shapef_{a}\mathbf{e}_i\right) \cdot \resMom\left(\velocityh,\pressh \right)\  \deriv \domain\\
		&+ \sum_{e=1}^{\nel} \int_{\domainE} \density \LSIC \left(\divergence \cdot \shapef_{a}\mathbf{e}_i\right) \resPre\left(\velocityh\right)\  \deriv \domain
	\end{split}
\end{align}
\noindent e:
\begin{align}
	...
	\end{split}
\end{align}
\noindent com $i=1,2$ para problemas 2D, $i=1,2,3$ para problemas 3D e $\mathbf{e}_i$ vetores unitários de base.

Considerando $\Acceleration$, $\Velocity$ e $\Press$ os vetores nodais dos graus de liberdade respectivos a aceralação, velocidade e pressão, pode-se escrever a forma semi discreta do problema da DFC como: encontrar $\Acceleration$, $\Velocity$ e $\Press$ de maneira que:
\begin{align}
...
A definição adequada dos parâmetros de estabilização tem papel fundamental no desempenho do método e na sua estabilidade numérica.

Desde os primeiros desenvolvimentos relacionados aos métodos estabilizados houve um amadurecimento das expressões de definição dos parâmetros $\tau$, as quais passam a levar em consideração formulações mais robustas, sendo adaptadas tanto para elementos de ordem elevadas, quanto para malhas mais complexas, como as usadas em AIG.

Considerando que neste trabalho dois tipos de aproximações espaciais são utilizadas, uma baseada no MEF e outra baseada em AIG, adotam-se os parâmetros propostos mais recentemente por \citeonline{OtoguroTT:2020}, \citeonline{TakizawaTO:2018} e \citeonline{TakizawaUT:2019}, que são adequados para ambas as aproximações. 

Para a obtenção dos parâmetros de estabilização é necessária a definição de um tensor métrico do elemento ($\matrixG$), a partir do qual serão definidos comprimentos direcionais no elemento. Com tal propósito, descreve-se inicialmente a matriz jacobiana $\matrixQ$, como:
\begin{align}
...
E o tensor métrico é então definido como:
\begin{align}
	\matrixG &= \matrixQhat^{-T} \matrixQhat^{-1}. \label{eq:tensor_metrico}
\end{align}

Em geral, se escolhe o espaço paramétrico baseado em razões como eficiência da integração numérica ou conveniência de implementação. A maioria das metodologias utilizadas para definir o comprimento do elemento não leva este fator em consideração. Para essa finalidade, em \citeonline{TakizawaTO:2018} apresenta-se a matriz de transformação ($\matrixD$) como:
\begin{align}
	\matrixD = \frac{\partial{\hat{\coordAdimen}}}{\partial{\coordAdimen}},
\end{align}
\noindent com $\hat{\coordAdimen}$ chamado de espaço paramétrico de preferência.

No contexto do MEF, para elementos simplex (triângulos e tetraedros para uma formulação bidimensional e tridimensional, respectivamente), buscando encontrar uma expressão que leve a um comprimento de elemento que não possua variação em função da ordenação dos nós, \citeonline{TakizawaUT:2019} introduziram um espaço paramétrico preferido, que consiste em um elemento simplex regular com distância entre vértices de 2, e chegaram a seguinte expressão para $\matrixD$ quando $\nsd = 2$:
\begin{align}
...
A partir do tensor métrico, é possível escrever o comprimento do elemento na direção do vetor $\rRGN$, como:
\begin{align}
	\RQD &=2\left(\rRGN\rRGN : \matrixG \right)^{-\frac{1}{2}},
\end{align}
\noindent onde o fator 2 vem de um típico espaço paramétrico, que é um quadrado (2D) ou um cubo (3D) com lado de comprimento 2, e $\rRGN$ é o vetor unitário na direção do gradiente da intensidade da velocidade, designado como:
\begin{align}
...
Além disso, nessa metodologia, o comprimento direcional do elemento ($\RQD$) é limitado pelos mínimos e máximos valores representados abaixo:
\begin{align}
...
\end{align}
\noindent onde $\lambda_{max}$ e $\lambda_{min}$ representam os autovalores máximos e mínimos da matriz $\matrixG$. 
...
\noindent e
\begin{align}
	\SUGNiii^{-1} = \kviscosity \left(\rRGN_{reg}\rRGN_{reg} : \matrixG + \left(1 - \lVert\rRGN_{reg}\rVert^2)4h_{min}^{-2} \right)\right),
\end{align}
...
\end{align}
\noindent com $\varepsilon$ sendo uma constante pequena e $\left(\lVert \gradient \lVert\velocityh- \velocityALEh\rVert\rVert\right)_0$ um valor de referência. Os termos $\SUGNi$, $\SUGNii$ e $\SUGNiii$ são parâmetros correspondentes aos termos convectivos, inerciais e viscosos, respectivamente.

\section{Integração temporal e solução numérica}\label{capitulo:Cap2:IntegTemp}

Para a integração temporal das equações governantes, utiliza-se o método $\alpha$-generalizado. Esse método foi proposto inicialmente por \citeonline{ChungH:1993} no contexto da Mecânica das Estruturas, estendido mais tarde para o contexto da Dinâmica dos Fluidos Computacional por \citeonline{JansenWH:2000}.

Considerando que o tempo da análise do problema é definido por um intervalo temporal de $[0,\totalTime]$, o qual é particionado em $npt$ subintervalos $\timeStep_{n} = t_{n+1} - t_{n}$, com $t_{n}$ e $t_{n+1}$ sendo os instantes anterior e atual, respectivamente, a solução do problema consiste em: conhecidos os valores nodais de aceleração, velocidade e pressão ($\Acceleration$, $\Velocity$ e $\Press$) no instante $n$, encontrar a solução no instante $n+1$ de forma que:
\begin{align}
...
\end{gather}
\noindent sendo $\Acceleration_{n+\alpham}$ e $\Velocity_{n+\alphaf}$ valores intermediários entre $t_{n}$ e $t_{n+1}$ do vetor aceleração e velocidade do escoamento. A relação entre os valores nodais de aceleração e velocidade é estabelecida de acordo com fórmula de Newmark (ver, por exemplo, \cite{Hughes:1976}):
\begin{gather}
...
Os parâmetros que definem o instante intermediário alteram as características de estabilidade e precisão do método. Segundo \citeonline{JansenWH:2000}, a precisão de segunda ordem é garantida, para problemas lineares, se: 
\begin{gather}
\gamma = 1/2 + \alpham - \alphaf,\label{eq:gamma}
\end{gather}
\noindent enquanto a estabilidade do problema é incondicional com:
\begin{gather}
...
O parâmetro $\specRadius$ é denominado raio espectral e quantifica a dissipação numérica introduzida pelo método para os modos de alta frequência, devendo estar contido no intervalo de $[0,1]$ para garantir estabilidade e controle do amortecimento numérico. Para $\specRadius = 0$ a dissipação de altas frequências é máxima e para $\specRadius = 1$ não há introdução de difusão numérica ao método.

Para a solução do sistema de equações não lineares compostas pela Equação~\eqref{eq:resid_QM_alpha} e pela Equação~\eqref{eq:resid_cont_alpha} utiliza-se o método de Newton-Raphson. O método pode ser separado em duas etapas: uma etapa preditiva (1 passo) e outra iterativa-corretiva (3 passos) \cite{BazilevsTT:2013a}.
...
Na fase 2, com os valores intermediários das variáveis nodais, resolve-se o sistema linear resultante da linearização da Equação~\eqref{eq:resid_QM_alpha} e da Equação~\eqref{eq:resid_cont_alpha} com respeito às variáveis de interesse $\Press_{n+1}$ e $\Acceleration_{n+1}$:
\begin{align}
...
\Velocity_{n+1}^{i+1} = \Velocity_{n+1}^{i} + \gamma \timeStep \Delta\Acceleration_{n+1}^{i},\label{eq:atu_vel}\\
\Press_{n+1}^{i+1} = \Press_{n+1}^{i} + \Delta\Press_{n+1}^{i}.\label{eq:atu_press}
\end{align}

Na utilização do método $\alpha$-generalizado, as integrais da Equação~\eqref{eq:resid_QM_alpha} e da Equação~\eqref{eq:resid_cont_alpha} são avaliadas no instante $t = t_{n+\alpha_{f}}$, de forma que:
\begin{align}
...
Finalmente, é possível estabelecer o algoritmo para a solução dos problemas de escoamentos incompressíveis com a formulação estabilizada do MEF, o qual é apresentado abaixo no Algoritmo \ref{alg:fluid}.

\begin{algorithm}
	\caption{Algoritmo para problemas da DFC}
	\label{alg:fluid}
	\begin{algorithmic}[1]
...
Para a verificação do código baseado no MEF são simulados dois exemplos amplamente estudados na literatura: escoamento sobre um cilindro e escoamento sobre uma cavidade quadrada.
...
\begin{figure}[!htbp]
	...
	\begin{center}
		...
		\subfloat[Malha \label{fig:cilindro_malha}]{\includegraphics[trim=0cm 1cm 0cm 1cm,clip,scale=0.4]{Imagens/Cap2/cilindro_malha.pdf}}
	\end{center}
	\legend{Fonte: Elaborada pela autora}
\end{figure}
...
Como pode-se observar na Figura~\ref{fig:cilindro_coefAero}, para $\Reynolds = 40$ o coeficiente de arrasto atinge um valor constante, quando o escoamento fica estacionário, permanecendo assim até o final da análise, enquanto o coeficiente de sustentação permanece nulo. Isso é o que se espera, visto que para Reynolds entre 5 à 50, deve-se observar a formação de dois vórtices simétricos e estacionários na região a jusante do cilindro \cite{Lienhard:1966}. 

Com o aumento do número de Reynolds, o par de vórtices é quebrado, gerando a chamada esteira de von Kármán, com formação de vórtices de maneira alternada entre as regiões superior e inferior do cilindro (\cite{Lienhard:1966}), levando ao comportamento oscilatório observado nos gráficos para $\Reynolds = 100$ e $\Reynolds=1000$. Os valores do coeficiente de Strouhal, para $\Reynolds = 100$ e $\Reynolds=1000$, assim como os valores médios obtidos para o coeficiente de arrasto ($C_{Dmed}$) para $\Reynolds = 40$, $\Reynolds = 100$ e $\Reynolds = 1000$ são apresentados na \autoref{tab:cilindro_CD_ST}, onde são comparados com os valores de referência provenientes do trabalho numérico de \citeonline{WANDERLEY2002445}. 

Vale ressaltar que os resultados apresentados para $\Reynolds=1000$ são idealizados, buscando reproduzir e verificar resultados numéricos de outros autores, visto que nesta faixa de número de Reynolds já existem efeitos de turbulência, que só poderiam ser captados com um modelo 3D.
...
\begin{table}[!htbp]
	\centering
	\caption{Comparação entre valores obtidos e os da referência}
	\begin{tabular}{ccccc}
		\toprule
...
Na Figura~\ref{fig:cilindro_camposVel} e na Figura~\ref{fig:cilindro_camposPressao} são apresentados os campos de velocidade e pressão ao longo do n-ésimo período de um ciclo de movimento periódico $nT$ para $\Reynolds = 100$. Pode-se notar, nessas imagens, a formação e o desprendimento de vórtices na esteira de Von Karmán.

\begin{figure}[!htbp]
	\caption{Cilindro: campo de velocidade para $\Reynolds = 100$}
	\centering
	\subfloat[$nT$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_vel2020.pdf}} \
	\subfloat[$nT + T/6$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_vel2030.pdf}} \\
	\subfloat[$nT + T/3$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_vel2040.pdf}} \
	\subfloat[$nT + T/2$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_vel2050.pdf}} \\
	\subfloat[$nT + 2T/3$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_vel2060.pdf}} \
	\subfloat[$nT + 5T/6$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_vel2070.pdf}} \\
...
\begin{figure}[!htbp]
	\caption{Cilindro: campo de pressão para $\Reynolds = 100$}
	\centering
	\subfloat[$nT$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_press2020.pdf}} \
	\subfloat[$nT + T/6$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_press2030.pdf}} \\
	\subfloat[$nT + T/3$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_press2040.pdf}} \
	\subfloat[$nT + T/2$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_press2050.pdf}} \\
	\subfloat[$nT + 2T/3$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_press2060.pdf}} \
	\subfloat[$nT + 5T/6$]{\includegraphics[scale=0.15,trim=5cm 5cm 5cm 6cm, clip=true]{Imagens/Cap2/cilindro_press2070.pdf}} \\
...
\subsection{Escoamento sobre cavidade} \label{capitulo:Cap2:VerApl:CavQuad}

Este exemplo considera uma cavidade hexaédrica com velocidade prescrita horizontal $u_{\infty}$ em sua face superior. A geometria do problema bem como as condições de contorno são apresentadas na Figura~\ref{fig:cavidade_geometria}. As paredes da cavidade são rígidas, com paredes laterais e do fundo com condição de aderência e, adicionalmente, condição de simetria na direção $y_3$. A discretização espacial em elementos finitos utilizada é apresentada na Figura~\ref{fig:cavidade_malha}, a qual consiste em 7252 elementos tetraédricos quadráticos e 14727 nós.  Embora este problema possa ser representado por uma discretização bidimensional, adota-se uma discretização 3D para um primeiro teste das implementações dos elementos finitos tetraédricos.

\begin{figure}[!htbp]
	\caption{Cavidade: geometria, condições de contorno e malha}
	\centering
	\subfloat[Geometria e condições de contorno\label{fig:cavidade_geometria}]{\includegraphics[scale=1.0,trim=0cm 0cm 0cm 0cm, clip=true]{Imagens/Cap2/cavidade_geometria.pdf}}  \ \ \ \ \ \ \ \ \
	\subfloat[Malha\label{fig:cavidade_malha}]{\includegraphics[trim=0 0 0 0,clip,scale=0.18]{Imagens/Cap2/cavidade_malha.png}}
	\legend{Fonte: Elaborada pela autora}
\end{figure}

O problema é estudado para três diferentes números de Reynolds (100, 400 e 1000), calculados de acordo com Equação~\eqref{eq:Reynolds}, considerando $L$ como sendo o comprimento do lado da cavidade. O problema foi simulado para uma velocidade na parede superior de $u_{\infty} = 1\text{,}0$m/s, $\density = 1\text{,}0$kg/m³, $\timeStep = 0\text{,}05$s, e $\specRadius = 0$, sendo a viscosidade do fluido variada de modo a alterar o número de Reynolds. A simulação foi mantida até que se atingiu o estado estacionário de escoamento. 

Os perfis de velocidade adimensionalizada ($\velocity/\velocinfty$) ao longo de duas linhas centrais nas direções $y_1$ e $y_2$, posicionadas no centro da espessura na direção $y_3$, são apresentados na Figura~\ref{fig:cavidade_graficos} e comparados com a referência de \citeonline{GhiaGS:1982}.

\begin{figure}[!htbp]
	\caption{Cavidade: perfis de velocidade adimensionalizados}
	\centering
	\subfloat[\label{fig:cavidade_Re100}$Re$=100]{\includegraphics[scale=0.7,trim=0.cm 0cm 0.3cm 0.2cm, clip=true]{Imagens/Cap2/cavidade_Re100.pdf}} \ \ \ \ \subfloat[\label{fig:cavidade_Re400}$Re$=400]{\includegraphics[scale=0.7,trim=0.cm 0cm 0.3cm 0.2cm, clip=true]{Imagens/Cap2/cavidade_Re400.pdf}} \
	\subfloat[\label{fig:cavidade_Re1000}$Re$=1000]{\includegraphics[scale=0.7,trim=0.cm 0cm 0.3cm 0.2cm, clip=true]{Imagens/Cap2/cavidade_Re1000.pdf}} \\ 
	\subfloat{\includegraphics[scale=1.]{Imagens/Cap2/cavidade_legenda.pdf}}
	\legend{Fonte: Elaborada pela autora}
	\label{fig:cavidade_graficos}
\end{figure}

Os campos de velocidade e de pressão, em um plano $y_1\times$$y_2$, para a situação estacionária são apresentados na Figura~\ref{fig:cavidade_vel} e na Figura~\ref{fig:cavidade_press}, respectivamente. Ressalta-se que para a simulação, por se tratar de um problema com todos os contornos com condição de Dirichlet impostos, a pressão torna-se indefinida. Por esse motivo, prescreveu-se pressão $\press = \press_{\infty} = 0\text{,}0$ no canto superior direito da cavidade. 

\begin{figure}[!htbp]
	\caption{Cavidade: campo de velocidade}
	\centering
	...
	\subfloat[\label{fig:cavidade_vel_Re1000}$Re$=1000]{\includegraphics[scale=0.20,trim=12cm 5.6cm 12cm 5cm, clip=true]{Imagens/Cap2/cavidade_velRe1000.pdf}}\\[3.0ex]
	% Barra de cores (legenda) separada
	\includegraphics[scale=0.25,trim=12cm 2cm 12cm 32.6cm, clip=true]{Imagens/Cap2/cavidade_velRe1000.pdf}
	\legend{Fonte: Elaborada pela autora}
	\label{fig:cavidade_vel}
\end{figure}

\begin{figure}[!htbp]
	\caption{Cavidade: campo de pressão}
	\centering
