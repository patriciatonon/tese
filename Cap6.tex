...
\begin{align}
	\domain = \globalModel \cup \localModel, 
\end{align}
\noindent e a zona de superposição $\overlappingZone$ é definida da seguinte forma:
\begin{align}
...
\begin{figure}[H]
	\caption{Domínio do modelo Arlequin}
	\centering 
...
\begin{align}
	(\lagrangeMultiplier,\Delta \velocity)_{\gluingZone} =  \int_{\gluingZone} k_{0}[\lagrangeMultiplier \cdot \Delta \velocity ] + k_{1}[\straintensor (\lagrangeMultiplier) : \straintensor (\Delta \velocity)] \ \deriv \gluingZone,
\end{align}
\noindent onde $\lagrangeMultiplier$ é o campo de multiplicadores de Lagrange, $\Delta \velocity = \uglobal |_{\gluingZone} - \ulocal |_{\gluingZone}$ é a diferença entre os campos acoplados na zona de colagem e $k_{0}$ e $k_{1}$ são constantes estritamente positivas. 

Quando $k_{0} > 0$ e $k_{1} = 0 $ tem-se o operador de acoplamento $L^{2}$. Esse operador estabelece a continuidade de ordem 0 do campo compatibilizado, o que significa que ele garante, de forma fraca, a continuidade das variáveis ao longo da zona de colagem. Para valores $k_{0} > 0$ e $k_{1} > 0 $ obtem-se o operador de acoplamento $H^{1}$, estabelecendo continuidade de ordem 1 do campo compatibilizado, garantindo, de forma fraca, a continuidade de uma combinação de variáveis e seu Laplaciano \cite{GuidaultAndBelytschko2007}.

O sucesso do método depende da escolha apropriada dos parâmetros $k_{0}$ e $k_{1}$. Para o acoplamento utilizando $L^{2}$, devido à simplicidade da aplicação da restrição dos campos compatibilizados na zona de colagem, o condicionamento do sistema depende fortemente do valor do parâmetro $k_{0}$, sendo esta uma das razões pela qual a maioria dos trabalhos realizados com o método Arlequin empregam o operador $H^{1}$. A obtenção de parâmetros ótimos para o método pode ser uma tarefa difícil, sendo esse um dos fatores que levaram \citeonline{FernandesEtAll:2020} ao desenvolvimento da técnica RBSAM, que será discutida na próxima seção.
...
Por fim, para que o método não adicione energia ao sistema, é necessário que seja definida uma função ponderadora, denominada $\arlequinWF$, que garanta a distribuição da energia do sistema ao longo dos modelos sobrepostos. Em geral, essa função é definida da seguinte forma:
\begin{align}
	\begin{cases} \arlequinWFGlobal \in [k_a;1] \ em \ \domain, \\ 
...
No caso deste trabalho, adotou-se para $\alpha_0$ e $\alpha_1$ na zona de colagem funções lineares da distância ao contorno $\Gamma_1$, de modo que:
\begin{align}
	\arlequinWFGlobal = 1 - \frac{(1-k_a)}{\delta_s}Y_{1}(\pos), \label{eq:funcPondeArleq_0}  
\end{align}
\begin{align}
	\arlequinWFLocal = \frac{(1-k_a)}{\delta_s}Y_{1}(\pos) \label{eq:funcPondeArleq_1}, 
\end{align}
\noindent com $Y_{1}(\pos)$ sendo a função distância assinalada medida a partir de $\Gamma_1$, com valores positivos dentro do domínio local e negativos fora, e $\delta_s$ representando a espessura definida para a zona de colagem.
...
A solução proposta por \citeonline{JamondD:2013} no trabalho foi a aplicação da condição de incompressibilidade em cada ponto do domínio computacional apenas uma vez. A metodologia consiste então em escolher um dos modelos no qual é removida a condição de incompressibilidade dos elementos total ou parcialmente localizados na zona de colagem ($\gluingZone$). Indiferente do modelo eleito para a remoção da condição de incompressibilidade na zona de colagem, na zona livre, a condição de incompressibilidade é removida do modelo global. Deve-se destacar que, no trabalho citado, existem algumas recomendações com relação à estabilidade da metodologia, como por exemplo, a necessidade de existir pelo menos um elemento global na zona livre. Tal trabalho não explora as possíveis mudanças que surgiriam na estabilidade numérica em caso de sucessivas remoções e inclusões da condição de incompressibilidade no caso de um modelo local móvel.

Por esse motivo, neste trabalho, opta-se pela adoção da formulação estabilizada.
Para a construção do Método Arlequin é necessário retornar às equações para um monomodelo (modelo usando apenas uma discretização espacial) apresentadas no \autoref{capitulo:Cap2}, as quais representam a forma fraca estabilizada das equações da quantidade de movimento e da continuidade. Entretanto, será usada inicialmente uma descrição euleriana das equações.

Considerando os espaços de dimensão finita das funções-tentativa que descrevem a velocidade ($\uArlequinSolution$) e a pressão ($\pArlequinSolution$), bem como seus respectivos espaços de funções-teste $\uArlequinTest$ e $\pArlequinTest$, com $i = 0,1$ indicando o índice do modelo, definidos como:
\begin{align}
...
\begin{align}
	\begin{split}
		...
		\sum_{e=1}^{\nel} \int_{\domainE} \PSPG \left(\frac{\gradient \qglobalh}{\density}\right) \cdot \resMomGlobal\left(\uglobalh,\pglobalh\right) \  \deriv \globalModel = 0,
...
O problema descrito pela Equação~\eqref{eq:QM_forma_fraca_0} até a Equação~\eqref{eq:lag_acopl} é a versão clássica do Método de Arlequin para o problema de Navier-Stokes estabilizado pela técnica PSPG/SUPG. Matematicamente, trata-se de um problema de ponto de sela decorrente de uma formulação mista. Entretanto, desde que a condição LBB seja satisfeita, existe solução para o problema e ela é única. 
...
O problema descrito no Método Arlequin clássico é análogo à formulação mista em elementos finitos para escoamentos incompressíveis, que limita a escolha das funções aproximadoras para o campo de velocidade e pressão. No caso da Mecânica dos Fluidos, conforme apresentado no \autoref{capitulo:Cap2}, uma forma de superar as restrições de LBB é o uso de métodos estabilizados, como o PSPG. Seguindo essa mesma filosofia,  \citeonline{FernandesEtAll:2020} introduzem uma técnica de estabilização consistente que será apresentada na seguinte seção. 

\section{Método Arlequin estabilizado aplicado a problemas de escoamentos incompressíveis}

Com intuito de superar a condição de LBB para o Método Arlequin, \citeonline{FernandesEtAll:2020} desenvolvem uma técnica de estabilização consistente baseada em resíduo. Para isso, introduz-se uma parcela adicional à equação dos campos de multiplicadores de Lagrange, que leva em conta o gradiente de $\lagrangeMultiplierWFh$ e o resíduo da equação da quantidade de movimento:
\begin{align}
...
\begin{align}
	...
	\end{split}
\end{align}
\noindent com os resíduos $\resMomI$ e $\resPreI$ escritos conforme a Equação~\eqref{eq:res_QM_i} e a Equação~\eqref{eq:res_C_i}.
...
Considerando $i=0,1$ designando os modelos global e local, respectivamente, tem-se a definição dos termos da Equação~\eqref{eq:sistema_result_ArlqRBSAM} como: $\mathbf{K}_{i}$ é uma matriz que representa os termos provenientes das matrizes referentes às equações da quantidade de movimento e da continuidade para o modelo $i$; $\mathbf{\hat{L}}_{i}$ é a matriz que representa os termos de acoplamento do modelo $i$; $\mathbf{L}_{i}^{T}$ é a matriz procedente dos termos da equação de restrição e de componentes da estabilização RBSAM do modelo $i$; $\mathbf{E}$ apresenta termos oriundos da estabilização RBSAM; $\mathbf{\bar{U}}_{i}$ representa os vetores nodais dos graus de liberdade respectivos a velocidade e pressão do modelo $i$; $\LagrangeMultiplier$ representa os graus de liberdade respectivos aos multiplicadores de Lagrange; $\mathbf{F}_i$ representa os vetores provenientes das equações da quantidade de movimento e da continuidade para o modelo $i$ e $\mathbf{F}_{\LagrangeMultiplier}$ representa os termos vetoriais advindos da estabilização RBSAM. 

Note-se que na estabilização Arlequin baseada no resíduo (RBSAM) não existem elementos zeros na diagonal da matriz, diferente do mesmo problema na formulação clássica Arlequin. No trabalho de \citeonline{FernandesEtAll:2020} pode-se encontrar a análise de estabilidade dessa técnica e testes numéricos que avaliam o condicionamento do sistema algébrico e a convergência do método.

O problema de Arlequin não linear apresentado na Equação~\eqref{eq:QM_forma_fraca_0_atu} até a Equação~\eqref{eq:lag_acopl_tARLQ} pode ser reescrito em sua forma semi-discreta residual, para $i=0,1$, da seguinte maneira:
\begin{align}
	\begin{split}
...
Considerando $\Acceleration_i$, $\Velocity_i$, $\Press_i$ e $\LagrangeMultiplier$ os vetores nodais dos graus de liberdade respectivos a aceleração, velocidade, pressão e multiplicadores de Lagrange, pode-se escrever o problema semi discreto da DFC como: determinar $\Acceleration_0$, $\Velocity_0$, $\Press_0$,$\Acceleration_1$, $\Velocity_1$, $\Press_1$ e $\LagrangeMultiplier$ de maneira que:
\begin{align}
...
Para a definição de $\tauArlequin$ toma-se como referência os trabalhos de \citeonline{TezduyarO:2000} e \citeonline{TezduyarS:2003}, nos quais se apresenta uma vasta quantidade de informação acerca das técnicas para obtenção dos parâmetros de estabilização das equações da DFC ($\SUPG$, $\PSPG$, $\LSIC$). 
...
\end{align}
\noindent sendo as variáveis da Equação~\eqref{eq:tAi} à Equação~\eqref{eq:tEi} as seguintes normas vetoriais:
\begin{align}
...
Quanto ao procedimento de integração temporal, utiliza-se o método $\alpha$-generalizado conforme a metodologia apresentada na \autoref{capitulo:Cap2:IntegTemp}; para a solução do sistema de equações não lineares composto pela Equação~\eqref{eq:resid_semi_discr_QM_0} até Equação~\eqref{eq:resid_semi_discr_acopla}, utiliza-se o método de Newton-Raphson. A solução resulta em uma etapa preditiva e outra iterativa-corretiva.
...
Na fase 2, com os valores intermediários das variáveis nodais resolve-se o sistema resultante da linearização da Equação~\eqref{eq:resid_semi_discr_QM_0} até Equação~\eqref{eq:resid_semi_discr_acopla} com respeito às variáveis de interesse $\Acceleration_{0(n+1)}$, $\Press_{0(n+1)}$,  $\Acceleration_{1(n+1)}$, $\Press_{1(n+1)}$ e $\LagrangeMultiplier_{(n+1)}$, calculando-se uma correção para essas variáveis:
\begin{align}
	\begin{bmatrix}
...
\begin{align}
	...
	\Velocity_{0(n+1)}^{i+1} = \Velocity_{0(n+1)}^{i} + \gamma \Delta t \Delta\Acceleration_{0(n+1)}^{i},\label{eq:atu_vel_0}\\
	...
	\Velocity_{1(n+1)}^{i+1} = \Velocity_{1(n+1)}^{i} + \gamma \Delta t \Delta\Acceleration_{1(n+1)}^{i},\label{eq:atu_vel_1}\\
	\Press_{1(n+1)}^{i+1} = \Press_{1(n+1)}^{i} + \Delta\Press_{1(n+1)}^{i}, \label{eq:atu_press_1} \\
	\LagrangeMultiplier_{(n+1)}^{i+1} = \LagrangeMultiplier_{(n+1)}^{i} + \Delta\LagrangeMultiplier_{(n+1)}^{i}. \label{eq:atu_lag_mult}
\end{align}
...
A Equação~\eqref{eq:QM_forma_fraca_0_atu} até Equação~\eqref{eq:lag_acopl_tARLQ} resolvem problemas de escoamentos incompressíveis em uma descrição euleriana. Entretanto, tem-se como objetivo a movimentação do domínio local do fluido (ver \autoref{fig:dominioArlequinMovel}) para acomodar a movimentação da estrutura nos problemas de IFE, ou para representar o deslocamento de um objeto imerso no escoamento.  Nota-se que o modelo global mantém sua geometria inalterada na mudança de passo de tempo, enquanto o modelo local é movimentado para representar uma nova localização de um objeto imerso. Vale ressaltar que o contorno do domínio do modelo local ($\localBoundary$) é conhecido em $t_n$ e em $t_{n+1}$ e que a zona de superposição $\overlappingZone$ é definida em diferentes posições em cada instante. Assim, faz-se uso de uma descrição ALE no modelo local ($\localModel$) enquanto o domínio global ($\globalModel$) mantém-se fixo e com descrição euleriana. 
...
Para análise de domínios móveis do tipo euleriano-ALE, a Equação~\eqref{eq:QM_forma_fraca_1_atu} é reescrita, levando-se em consideração as definições apresentadas na \autoref{capitulo:Cap2:ALE}, resultando em:
\begin{align}
	\begin{split}
	...
	\end{split}
\end{align}
\noindent enquanto o resíduo apresentado na Equação~\eqref{eq:res_QM_i} passa a ser escrito para $i = 1$, como:
\begin{align}
...
A solução de modelos com fronteiras móveis requer uma técnica adequada para o movimento da malha local. Neste trabalho, emprega-se o método descrito em \citeonline{Tononetal:2021}, baseado na estratégia Mesh-Jacobian Based Stiffening (MJBS), originalmente proposta por \citeonline{TezduyarBSJ:1992c}. Detalhes adicionais sobre a formulação e implementação são apresentados na \autoref{capitulo:Cap7:CondAcop:MovMalha}.
...
A etapa 1 consiste em calcular a distância assinada $Y_1(\pos)$ de cada ponto — sejam nós da malha de elementos finitos ou pontos de controle da malha isogeométrica — em relação ao contorno $\localBoundary$ de cada modelo.
Na etapa 2, com base nessa distância, identificam-se os elementos locais que compõem a zona de colagem, de acordo com a espessura $\delta_s$ previamente definida pelo usuário. Somente são considerados pertencentes a essa região os elementos cujos nós se encontram integralmente dentro da zona de colagem. A função ponderadora para os modelos (etapa 3) é  determinada de acordo com as relações expressas na Equação~\eqref{eq:funcPondeArleq}, Equação~\eqref{eq:funcPondeArleq_0} e Equação~\eqref{eq:funcPondeArleq_1}. 
...
O algoritmo que descreve a implementação computacional é apresentado no Algoritmo \ref{alg:fluidARLQ}. A implementação computacional e resolução do sistema de equações resultantes ocorreu de forma análoga ao monomodelo descrito na \autoref{capitulo:Cap2:ImpleComput}. O índice $i$ apresentado diz respeito à iteração de Newton-Raphson.

\begin{algorithm}
	\caption{Algoritmo para problemas móveis da DFC utilizando técnica RBSAM}
	\label{alg:fluidARLQ}
	\begin{algorithmic}[1]
		\For {o passo de tempo $0$ até $npt-1$} 
		\State $i\gets 0$;
		\State Predição da solução: aplicação da Equação~\eqref{eq:pred_acel_0}, Equação~\eqref{eq:pred_vel_0}, Equação~\eqref{eq:pred_press_0},
		Equação~\eqref{eq:pred_acel_1}, Equação~\eqref{eq:pred_vel_1}, Equação~\eqref{eq:pred_press_1} e Equação~\eqref{eq:pred_lag_mult};
		...
		\State Definição da função de ponderadora de acordo com Equação~\eqref{eq:funcPondeArleq}, Equação~\eqref{eq:funcPondeArleq_0} e Equação~\eqref{eq:funcPondeArleq_1};
		\State Busca pela correspondência entre os pontos de integração da malha local na malha global;
		\State Interpolação das variáveis do problema: aplicação da Equação~\eqref{eq:inter_acel_0}, Equação~\eqref{eq:inter_vel_0}, Equação~\eqref{eq:inter_press_0},
		Equação~\eqref{eq:inter_acel_1}, Equação~\eqref{eq:inter_vel_1}, Equação~\eqref{eq:inter_press_1} e Equação~\eqref{eq:inter_lag_mult}; 
		\State Cálculo da correção das variáveis do problema: resolução do sistema apresentado na Equação~\eqref{eq:sistema_linear_Arlq};
		\State Atualização da solução: cálculo de acordo com Equação~\eqref{eq:atu_acel_0}, Equação~\eqref{eq:atu_vel_0}, Equação~\eqref{eq:atu_press_0},
		Equação~\eqref{eq:atu_acel_1}, Equação~\eqref{eq:atu_vel_1}, Equação~\eqref{eq:atu_press_1} e Equação~\eqref{eq:atu_lag_mult};
...
\section{Verificação e aplicações}\label{capitulo:Cap6:Exemplos}

Para a verificação da implementação do Método Arlequin estabilizado, dois exemplos amplamente explorados nas bibliografias são simulados. O primeiro exemplo trata do escoamento sobre um aerofólio NACA 0012 fixo, enquanto o segundo, com intuito de verificar o método aplicado a problemas com contornos móveis, consiste no mesmo aerofólio NACA 0012, porém com um movimento de arfagem prescrito. 

\subsection{Escoamento sobre aerofólio NACA 0012 fixo - Método Arlequin} \label{capitulo:Cap6:Exemplos:NACA0012}

Considera-se um aerofólio NACA 0012 com corda de comprimento adimensional unitário e ângulo de ataque de $10^{\circ}$, submetido a um escoamento incompressível viscoso com Reynolds ($\Reynolds$) 1000, calculado a partir da dimensão do aerofólio e da velocidade de entrada do escoamento. Foram utilizados ainda como parâmetros de análise: $u_{\infty} = 1\text{,}0$,  $\density = 1\text{,}0$,  $\timeStep = 0\text{,}02$ e $\specRadius = 0\text{,}75$. O domínio do problema é apresentado na \autoref{fig:aerofolio_geometria}, onde são indicadas as dimensões em termos do comprimento da corda do aerofólio. Na face esquerda o fluido entra com velocidade $u_{\infty}$ na direção $y_1$. Os contornos superior e inferior possuem condição de contorno de parede lisa, enquanto na face da direita considera-se força de superfície nula ($p_\infty=0$). 

\begin{figure}[!htbp]
	\caption{Aerofólio: geometria}
	\centering 
	{\includegraphics[scale=0.9,trim=0cm 0cm 0cm 0cm, clip=true]{Imagens/Cap6/aerofolio_geometria.pdf}}	
	\label{fig:aerofolio_geometria}
	\legend{Fonte: Elaborada pela autora}
\end{figure}

Para fins de comparação, simula-se inicialmente o problema com uma malha única de elementos finitos, representando o monomodelo. O monomodelo é discretizado com 9240 elementos triangulares quadráticos e 18792 nós (\autoref{fig:aerofolio_malhaMonoFEM}). 

Para o Método Arlequin, utiliza-se uma discretização global isogeométrica (AIG) e uma malha local, mais refinada, em elementos finitos (MEF), como apresentado na Figura~\ref{fig:aerofolio_malhaArlequin} com detalhe da discretização local na Figura~\ref{fig:aerofolio_detalheLocal}. A malha global isogeométrica é constituida por 9 $patches$, totalizando 15561 pontos de controle e 14025 células. A malha local, por sua vez, é composta por 5214 elementos triangulares quadráticos e 10670 nós. 

Na Figura~\ref{fig:aerofolio_detalheLocal} pode-se observar em vermelho os elementos que fazem parte da zona de colagem. A espessura da zona de colagem foi definida como $0\text{,}2$, compreendendo 628 elementos triangulares quadráticos e 1428 nós. 

\begin{figure}[!htbp]
	\caption{Aerofólio: malha monomodelo (MEF)}
	\centering
	{\includegraphics[scale=2.5,trim=0cm 0.9cm 0cm 0.9cm, clip=true]{Imagens/Cap6/aerofolio_malhaMonoFEM.pdf}}	
...
\begin{figure}[!htbp]
	...
	\subfloat[Discretização da malha local\label{fig:aerofolio_detalheLocal}]{\includegraphics[trim=0 0 0 0,clip=true,scale=1.2]{Imagens/Cap6/aerofolio_detalheLocal.pdf}}
	\label{fig:aerofolio_malhaArlequin_detalheLocal}
	\legend{Fonte: Elaborada pela autora}
\end{figure}

Nesse problema, observa-se o surgimento de uma esteira de vórtices a jusante do aerofólio, que resulta em um número de Strouhal ($\Strouhal$) equivalente a $0\text{,}877$. Esse valor encontra-se em concordância com o obtido por \citeonline{MittalT:1994}, de $0\text{,}862$. 

Na Figura~\ref{fig:aerofolio_CD} e na Figura~\ref{fig:aerofolio_CL} apresentam-se os resultados de coeficiente de arrasto e de sustentação obtidos para as análises realizadas. Pode-se observar que os resultados obtidos com o modelo baseado no Método Arlequin estabilizado estão de acordo com os obtidos para o monomodelo.
...
Na Figura~\ref{fig:aerofolio_velocidade} e na Figura~\ref{fig:aerofolio_pressao} estão apresentados os campos de velocidade e pressão, respectivamente, para o período de um ciclo de desprendimento de vórtices. 

\begin{figure}[!htbp]
	\caption{Aerofólio: campo de velocidade}
	\centering
	\subfloat[$nT$]{\includegraphics[scale=0.13,trim=0cm 4cm 0cm 4cm, clip=true]{Imagens/Cap6/aerofolio_velTn.pdf}} \ \
	\subfloat[$nT + T/4$]{\includegraphics[trim=0cm 4cm 0cm 4cm,clip=true,scale=0.13]{Imagens/Cap6/aerofolio_velTn4.pdf}} \\
	\subfloat[$nT + T/2$]{\includegraphics[scale=0.13,trim=0cm 4cm 0cm 4cm, clip=true]{Imagens/Cap6/aerofolio_velTn2.pdf}} \ \
	\subfloat[$nT + 3T/4$]{\includegraphics[trim=0cm 4cm 0cm 4cm,clip=true,scale=0.13]{Imagens/Cap6/aerofolio_velTn34.pdf}} \\
	{\includegraphics[trim=0cm 0.2cm 0cm 0cm,clip=true,scale=0.4]{Imagens/Cap6/aerofolio_velLegenda.png}}
...
\begin{figure}[!htbp]
	...
	\centering
	\subfloat[$nT$]{\includegraphics[scale=0.13,trim=0cm 4cm 0cm 4cm, clip=true]{Imagens/Cap6/aerofolio_pressTn.pdf}} \ \
	\subfloat[$nT + T/4$]{\includegraphics[trim=0cm 4cm 0cm 4cm,clip=true,scale=0.13]{Imagens/Cap6/aerofolio_pressTn4.pdf}} \\
	\subfloat[$nT + T/2 $]{\includegraphics[scale=0.13,trim=0cm 4cm 0cm 4cm, clip=true]{Imagens/Cap6/aerofolio_pressTn2.pdf}} \ \
	\subfloat[$nT + 3T/4$]{\includegraphics[trim=0cm 4cm 0cm 4cm,clip=true,scale=0.13]{Imagens/Cap6/aerofolio_pressTn34.pdf}}\\
...
\subsection{Escoamento sobre aerofólio com movimento de arfagem prescrito}

Para a verificação computacional da técnica Arlequin estabilizada aplicada a problemas com contornos móveis  considera-se o mesmo aerofólio~da \autoref{capitulo:Cap6:Exemplos:NACA0012}, porém agora com um movimento de arfagem oscilatório. O aerofólio apresenta variação do ângulo de ataque em $20^{\circ}$, iniciando o movimento em $10^{\circ}$ e finalizando-o em $30^{\circ}$. 

Para descrever-se tal movimento aplica-se o movimento de rotação de corpo rígido sobre o ponto médio da corda, de acordo com a relação:
\begin{align}
	\theta = \frac{\theta_{max}+\theta_{min}}{2} - \frac{\theta_{max}-\theta_{min}}{2}\cos{\omega_{f}t},
\end{align}
\noindent com $\omega_{f} = 2\pi f_{o}$, sendo $f_{o}$ a frequência de oscilação, adotada nesse estudo como $1\text{,}0$, $\theta_{max} = 30^{\circ}$ e $\theta_{min} = 10^{\circ}$.

As dimensões da geometria do domínio computacional são alteradas (ver \autoref{fig:aerofolioMov_geometria}) para capturar os efeitos do desprendimento de vórtices que, para esse problema, ocorrem em uma faixa mais ampla. Os demais parâmetros de análise foram mantidos iguais aos apresentados no exemplo da \autoref{capitulo:Cap6:Exemplos:NACA0012}. 
...
O monomodelo emprega uma malha com 12438 elementos triangulares quadráticos e 25188 nós. As discretizações global e local para o Método Arlequin são as mesmas do problema da \autoref{capitulo:Cap6:Exemplos:NACA0012}, incluindo a quantidade de elementos na zona de colagem.

É importante ressaltar que, para iniciar a simulação desse exemplo, utilizou-se como campo inicial de velocidade e pressão, valores obtidos em uma solução de longo termo do aerofólio na condição de repouso.

A variação dos coeficientes de arrasto e sustentação ao longo do tempo são apresentados nas \autoref{fig:aerofolioMov_CD} e na \autoref{fig:aerofolioMov_CL}. Nota-se nas imagens que o monomodelo e o modelo Arlequin estão consistentes em suas respostas. Soluções semelhantes podem ser observados no trabalho de \citeonline{Fernandes:2020}.
...
\begin{figure}[!htbp]
	...
	\centering 
	\includegraphics[scale=0.8,trim=0cm 0cm 0cm 0cm, clip=true]{Imagens/Cap6/aerofolioMov_CL.pdf}	
	\label{fig:aerofolioMov_CL}
	\legend{Fonte: Elaborada pela autora}
\end{figure}

Na Figura~\ref{fig:aerofolioMov_velocidade} e na Figura~\ref{fig:aerofolioMov_pressao} são apresentados os campos de velocidade e pressão em diferentes instantes dentro de um período de ciclo do movimento oscilatório prescrito.

\begin{figure}[!htbp]
	\caption{Aerofólio Mov.: campo de velocidade}
	\centering
...
\begin{figure}[!htbp]
	\caption{Aerofólio Mov.: campo de pressão}
	\centering
